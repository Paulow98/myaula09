...
// function that defines the world, placing everything on your place, allocating all the variables that need to be allocated,
// sorting all the agents at random places. Receives as argument an ini_t variable
...
//function that makes the movement of a playable agent. Receives as arguments the agent that wil move, the world where he is at and the direction that he will move.Works for both human and zombie
...
// just a function that uses of range to the agent to react, not used
...
// function that moves a npc agent, works with both human and zombie. If it is a zombie, it follows the nearest human, if it is a human, runs away from the nearest zombie.
// receives as argument the agent that will move and the world that he is at.
...
// function that returns in wich section of the map the agent passed as argument is at. Used in the findCloser function and move.
/*	1|2
	---
	4|3
*/
...
// function that searches the nearby agent of the opposite type of the agent passed as the argument.Receives the agent and the world he is at.
...
// returns the distance between the position of the agent p1 and agent p2, doesnt matter where he is
...
// the same as the last one but instead of receiving POS it receives AGENT as arguments
...
// returns the lowest distance between two agents(if is the distance one or the worldDistance one)
...
// returns the distance between two positions on the grid
int distance(POS p1, POS p2)
{
	return sqrt(pow(p2.x - p1.x, 2) + pow(p2.y - p1.y, 2));
}
// returns the distance between two agents on the grid
int distance(AGENT p1, AGENT p2)
{
	return sqrt(pow(p2.pos.x - p1.pos.x, 2) + pow(p2.pos.y - p1.pos.y, 2));
}
// function that checks if the second agent is close enough of the first agent, if true, infects the second one, transforming him into a zombie
...
// update the info of the agent passed as the argument in the array of agents
...
// shuffles the order of all agents of the agent array
